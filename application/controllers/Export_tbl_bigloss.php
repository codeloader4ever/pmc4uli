...
class Export_tbl_bigloss extends CI_Controller {
    function __construct(){
        parent::__construct();
        //$this->load->library(array('PHPExcel','PHPExcel/IOFactory'));
        $this->load->model('tbl_bigloss'); // memanggil model mread
    }
    public function export(){
        $ambildata = $this->tbl_bigloss->export();
        ...
        if(count($ambildata)>0){
            ...
            $val = array("Id Big Loss","Big Loss Name","Big Loss Desc");
            ...
            foreach ($ambildata as $frow){
				...
				$objset->setCellValue("A".$baris, $frow->id_bigloss);
                $objset->setCellValue("B".$baris, $frow->name); 
				$objset->setCellValue("C".$baris, $frow->description); 
                //Set number value
